...
- **Tests de integración con tokens de Guid random están rotos** (`PersonsEndpointTests` x8 y `CasonaStayEndpointTests` x8 fallan en `dev-backend`). Desde el chequeo de sesión activa en `OnTokenValidated` (`Program.cs`), el JWT se valida contra un usuario real en base (Status `Active` + `token_version`), y esas clases firman tokens con `Guid.NewGuid()` que no existe en `Users` → 401. El patrón correcto es `SembrarActorAsync` (crear usuario `Active` real y firmar el token con su Id), como ya hacen `AuthControllerTests`, `SocialRecordsEndpointTests`, `UserApprovalEndpointTests` y `AttendanceEndpointTests`. Detectado al correr la suite completa al implementar SCRUM-135; no se tocaron por estar fuera de alcance.
