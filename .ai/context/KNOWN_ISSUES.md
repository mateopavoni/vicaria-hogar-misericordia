...
- **El botón "Nueva ficha" del listado de fichas no navegaba (frontend, encontrado después de esa pasada):** los dos `routerLink="/dashboard/ficha-nueva"` (header y estado vacío) apuntaban a una ruta inexistente y la wildcard `**` estaba comentada — el click no hacía nada (fallo silencioso, solo error de consola). Se corrigió el link a `dashboard/fichas/crear`, se agregó un redirect `ficha-nueva` → `fichas/crear` (compatibilidad) y se activó la `**` → `NotFoundComponent` para que links muertos futuros muestren 404 en vez de fallar en silencio.
