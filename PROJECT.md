...
**Nota (2026-09-18):** el trabajo de estadías en casona y egreso de residente (SCRUM-140/146, ya implementado en `dev-backend`) se integra a **EP-12** — no tenía épica asignada en versiones anteriores de esta tabla. Ver `.ai/context/DOMAIN.md` y `.ai/context/DECISIONS.md` para el detalle y para revertir esta clasificación si el equipo prefiere una épica propia.
...
**Nota (2026-09-18):** esta sección describe el estado al 2026-08-25 y quedó desactualizada (ej. la migración a SQL Server y el bloqueo por 5 intentos, listados acá como pendientes, ya están completos). Se deja sin reescribir para no perder el historial de esta rama — ver `.ai/context/CURRENT_STATE.md` para una foto verificada más reciente, comparando `main`/`dev-backend`/`dev-frontend` por separado.

**Último relevamiento del código real (rama `dev-backend`, 2026-08-25):**

✅ **Completo:**
- RF-01 (registro y aprobación de cuenta): registro, listado de pendientes, aprobar, rechazar, con auditoría.
- RF-02 (login): `LoginAsync`, `POST /api/auth/login`, emisión de JWT (token + user), 401 credenciales inválidas, 403 cuenta no aprobada. Probado end-to-end contra Postgres real (registro → aprobación → login).
- RF-03 (roles diferenciados): 3 roles definidos y autorización por rol probada con tests. Tabla de permisos independiente (`Permission`/`RolePermission`) para el rol Directora de Casona en revisión (PR #7, SCRUM-88) — falta el filtrado de fichas por rol (no existe la entidad `Ficha` todavía) y el alta desde un panel de usuarios (no hay panel admin en el frontend).
- Entidades base: `Usuario`, `Rol`, `AuditLog`, `Permission`, `RolePermission` (esta última en PR sin mergear).

❌ **Pendiente:**

- **RF-02 — Bloqueo tras 5 intentos fallidos:** ticket creado (SCRUM-103, backend), faltan los campos `IntentosFallidos` y `BloqueadoHasta` en la entidad `Usuario` (sí están en el diagrama de clases original).
- **RF-02 — Expiración de sesión por inactividad:** SCRUM-96, sin arrancar. Requiere decisión de diseño (refresh token vs. tracking de actividad).
...
- **CORS:** no configurado — el login se probó vía el proxy de `ng serve` (mismo origen), que esconde el problema en dev. Sigue bloqueante para integrar en producción.
